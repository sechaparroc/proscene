...
public class Camera extends Eye implements Copyable {
 ...
 @Override public void setUpVector(Vec up, boolean noMove) {
 ...
 @Override public Vec viewDirection() {
 ...
 @Override public void setOrientation(Rotation q) {
 ...
 @Override public float sceneToPixelRatio(Vec position) {
  switch (type()) {
  case PERSPECTIVE:
 ...
 @Override public boolean isPointVisible(Vec point) {
 ...
 @Override public Visibility ballVisibility(Vec center, float radius) {
  if (!gScene.areBoundaryEquationsEnabled())
 ...
 @Override public Visibility boxVisibility(Vec p1, Vec p2) {
  if (!gScene.areBoundaryEquationsEnabled())
 ...
 @Override public float[][] computeBoundaryEquations() {
  return computeBoundaryEquations(new float[6][4]);
 }

 @Override public float[][] computeBoundaryEquations(float[][] coef) {
  // soft check:
 ...
 @Override public boolean setAnchorFromPixel(Point pixel) {
 ...
 @Override public boolean setSceneCenterFromPixel(Point pixel) {
  Vec pup = pointUnderPixel(pixel);
  if (pup != null) {
   setSceneCenter(pup);
   return true;
 ...
 @Override public void computeView() {
 ...
 @Override public void computeProjection() {
  float ZNear = zNear();
  float ZFar = zFar();
 ...
 @Override public void fromView(Mat mv, boolean recompute) {
  Quat q = new Quat();
  q.fromMatrix(mv);
  setOrientation(q);
 ...
 @Override public void lookAt(Vec target) {
 ...
 @Override public Vec at() {
  return Vec.add(position(), viewDirection());
 }

 @Override public void fitBall(Vec center, float radius) {
  float distance = 0.0f;
  switch (type()) {
  case PERSPECTIVE: {
 ...
 @Override public void fitBoundingBox(Vec min, Vec max) {
 ...
 @Override public void fitScreenRegion(Rect rectangle) {
 ...
 @Override public void interpolateToZoomOnPixel(Point pixel) {
