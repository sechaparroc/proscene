...
public class Rot implements Rotation {
 ...
 @Override public Rotation inverse() {
  return new Rot(-angle());
 }

 @Override public Vec rotate(Vec v) {
  float cosB = (float) Math.cos(angle());
  float sinB = (float) Math.sin(angle());
  return new Vec(((v.x() * cosB) - (v.y() * sinB)), ((v.x() * sinB) + (v.y() * cosB)));
 }

 @Override public Vec inverseRotate(Vec v) {
  float cosB = (float) Math.cos(-angle());
  float sinB = (float) Math.sin(-angle());
  return new Vec(((v.x() * cosB) - (v.y() * sinB)), ((v.x() * sinB) + (v.y() * cosB)));
 }

 @Override public Mat matrix() {
 ...
 @Override public Mat inverseMatrix() {
  float cosB = (float) Math.cos(-angle());
  float sinB = (float) Math.sin(-angle());
 ...
 @Override public void fromMatrix(Mat glMatrix) {
 ...
 @Override public final void compose(Rotation r) {
 ...
 @Override public float normalize() {
