...
public class Quat implements Linkable, Rotation {
 ...
 @Override public void link(float[] src) {
  quat = src;
 }
 ...
 @Override public float[] get(float[] target) {
 ...
 @Override public void set(float[] source) {
  if (source.length == 4) {
   quat[0] = source[0];
   quat[1] = source[1];
   quat[2] = source[2];
 ...
 @Override public void set(Linkable q) {
  ...
  */
 @Override public final void negate() {
  this.quat[0] = -this.quat[0];
  this.quat[1] = -this.quat[1];
  this.quat[2] = -this.quat[2];
 ...
 @Override public final void compose(Rotation q) {
  if (q instanceof Quat)
   multiply((Quat) q);
  else {
  ...
  * @see #invert()
  */
 @Override public final Quat inverse() {
  ...
  */
 @Override public final float normalize() {
  ...
  * @param v the Vec
  */
 @Override public final Vec rotate(Vec v) {
  ...
  * @param v the Vec
  */
 @Override public final Vec inverseRotate(Vec v) {
  ...
  * @see #fromAxisAngle(Vec, float)
  */
 @Override public void fromTo(Vec from, Vec to) {
  ...
  * @see #fromRotatedBasis(Vec, Vec, Vec)
  */
 @Override public final void fromMatrix(Mat glMatrix) {
  ...
  * @see #axis()
  */
 @Override public final float angle() {
  ...
  */
 @Override public final Mat matrix() {
  float q00 = 2.0f * this.quat[0] * this.quat[0];
  float q11 = 2.0f * this.quat[1] * this.quat[1];
  float q22 = 2.0f * this.quat[2] * this.quat[2];
  ...
  */
 @Override public final Mat inverseMatrix() {
  Quat tempQuat = new Quat(this.quat[0], this.quat[1], this.quat[2], this.quat[3]);
  tempQuat.invert();
